...
import com.kriogenik.guzeeva.state.person.PersonNotCreatedState
...
@SpringBootTest
class PersonNotCreatedStateTest {

    @Autowired
    private lateinit var personNotCreated: PersonNotCreatedState
    ...
    fun changeState(){
        ...
        val resultState  = Person(state = PersonEntityState.NOT_CREATED)
        ...
        assert(personNotCreated.execute(initialState) == Optional.of(resultState))
    }
    ...
    fun changeStateError(){
        ...
        assert(personNotCreated.execute(initialState) == Optional.empty<Person>())
    }
