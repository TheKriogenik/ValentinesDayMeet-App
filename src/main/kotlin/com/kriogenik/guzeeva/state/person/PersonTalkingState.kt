...
@Component
class PersonTalkingState: State<Person> {

    override val state: EntityState<Person> = PersonEntityState.TALKING
    ...
    override fun execute(context: Person): Optional<Person> {
        ...
        return context.copy(state = PersonEntityState.TALKING).let(service::update).map {
