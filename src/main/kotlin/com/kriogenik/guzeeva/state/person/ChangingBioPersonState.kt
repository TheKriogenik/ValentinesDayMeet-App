...
@Component
class ChangingBioPersonState: State<Person> {

    override val state: EntityState<Person> = PersonEntityState.CHANGING_BIO
    ...
    override fun execute(context: Person): Optional<Person> {
        ...
        return context.copy(state = PersonEntityState.CHANGING_BIO).let(service::update).map {
