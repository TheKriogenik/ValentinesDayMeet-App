...
class ChangingAgePersonState: State<Person> {

    override val state: EntityState<Person> = PersonEntityState.CHANGING_AGE
    ...
    override fun execute(context: Person): Optional<Person> {
        ...
        return context.copy(state = PersonEntityState.CHANGING_AGE).let(service::update).map {
