...
import com.kriogenik.guzeeva.state.person.PersonTalkingState
...
@SpringBootTest
class PersonTalkingStateTest {

    @Autowired
    private lateinit var personActiveState: PersonTalkingState
    ...
    fun changeState(){
        ...
        val resultState  = Person(state = PersonEntityState.TALKING)
    ...
    fun changeStateError(){
        val initialState = Person(state = PersonEntityState.TALKING)
        Mockito.`when`(personService.find(initialState.vkId))
