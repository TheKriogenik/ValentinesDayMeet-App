...
public class LongPoll {
    ...
    public LongPoll(Client client, Integer last_ts) {
        this.ts = last_ts;
    ...
    private boolean setData(Integer need_pts, Integer version, Double API, Integer wait, Integer mode) {
        ...
        this.ts = this.ts == null ? serverResponse.getTs() : this.ts;
    ...
    private void startListening() {
        ...
        while (longpollIsOn) {
            ...
            if (response.has("failed")) {
                ...
                LOG.warn("Response of VK LongPoll fallen with error code {}", code);
                ...
                if (this.updatesHandler.callbacksCount() > 0 || this.updatesHandler.commandsCount() > 0 || this.updatesHandler.chatCallbacksCount() > 0) {
                    ...
                    if (response.has("ts") && response.has("updates")) {
                        ...
                        this.writeText("last_ts.tmp", String.valueOf(response.getInt("ts")));

                        LOG.debug("Saved ts = {} for application reload", response.getInt("ts"));
    ...
    private void writeText(String path, String data) {
        try (FileWriter fw = new FileWriter(new File(path), false)) {
            ...
            LOG.error("Problem with {} writing {}: {}", path, data, e.getMessage());
        }
    }
}